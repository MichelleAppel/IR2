...
in finding a reasonable configuration for the positional language model,
not in proofing that one kernel/parameter combination is better than another.
Thus, based on the results shown in Table \ref{tbl_plm}  we use for the rest 
of the experiment the positional language model with a passage kernel and 
dirichlet smoothing parameter value 500.
...
In Section \ref{ResultsLexicalModels} we calculated the nDCG@10 scores
for each retrieval function averaged over the querie results.
